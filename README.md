...
##### 编译源码
找到 dobin-pom 工程下的 pom.xml，执行 maven clean install 命令进行一键打包。
一般来说不会有什么问题，如果还打包失败，可以按照优先级逐个编译试一试。

##### 导入数据库
新建dobin数据库，使用项目sql目录下的dobin.sql 脚本，导入初始化数据库。
修改 dobin-admin 下 application.yml 中的数据源配置信息为自己的数据库配置。
修改 dobin-backup下 application.yml 中的数据源配置信息为自己的数据库配置。

##### 启动系统

###### 基础必需模块（注册中心：dobin-consul，服务监控：dobin-monitor）

找到 dobin-consul 工程，根据安装说明安装注册中心，执行 consul agent -dev 启动。
找到 dobin-monitor 工程下的MangoMonitorApplication， 启动项目，开启服务监控。

###### 权限管理模块（权限管理：dobin-admin，备份还原：dobin-backup）
找到 dobin-admin 工程下的MangoAdminApplication， 启动项目，开启权限系统服务。
找到 dobin-backup 工程下的MangoBackupApplication.java，启动项目，开启备份服务。
...
dobin-producer： 服务提供者示例，演示服务提供者的实现

dobin-consumer： 服务消费者示例，演示服务消费者的实现

dobin-hystrix： 服务熔断监控模块，演示熔断监控功能的实现

dobin-zuul： API服务网关模块，演示API统一网关的实现

dobin-config： 配置中心服务端，演示分布式配置中心的实现

##### 注意事项：
注册中心是基础服务，需要先安装Consul，找到dobin-consul工程，根据安装说明安装Consul。

如果需要链路追踪服务，需要安装zipkin，找到dobin-zipkin 工程，根据安装说明安装zipkin。

如果需要配置中心服务，需要安装rabbitMQ，找到dobin-config 工程，根据安装说明安装rabbitMQ。
